...
echo "Fetching newsletter digests from AWS S3..."

# Get list of digest files from S3, sort by date (newest first), limit to 10
digests=$(aws s3 ls s3://alvarobp-digests/newsletters-digests/ --recursive | \
    grep '\.html$' | \
    sort -k1,2 -r | \
    head -10 | \
    awk '{print $4}')
...
selected=$(echo -n "$digest_options" | gum choose --header "Select a newsletter digest to open:")
